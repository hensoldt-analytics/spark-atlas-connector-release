...
object ExtractFromDataSource extends Logging {
  ...
  private val CLASS_NAME_KAFKA_CONTINUOUS_DATA_READER_FACTORY =
    "org.apache.spark.sql.kafka010.KafkaContinuousDataReaderFactory"

  def extractTopic(writer: InternalRowMicroBatchWriter): Option[KafkaTopicInformation] = {
    ...
    val writerFactory = writer.createInternalRowWriterFactory()
  ...
  def extractSourceTopicsFromDataSourceV2(r: DataSourceV2ScanExec): Seq[KafkaTopicInformation] = {
    ...
    r.inputRDDs().foreach(rdd => rdd.partitions.foreach {
      case e: DataSourceRDDPartition[_] =>
        if (isKafkaContinuousDataReaderFactory(e.readerFactory)) {
          populateValuesFromKafkaContinuousDataReaderFactory(e.readerFactory) match {
  ...
  private def isKafkaContinuousDataReaderFactory(factory: DataReaderFactory[_]): Boolean = {
    belongToTargetClass(CLASS_NAME_KAFKA_CONTINUOUS_DATA_READER_FACTORY, factory)
  }
  ...
  def populateValuesFromKafkaContinuousDataReaderFactory(
      readerFactory: DataReaderFactory[_]): Option[(String, Option[String])] = {
    if (isKafkaContinuousDataReaderFactory(readerFactory)) {
      val topic = ReflectionHelper.reflectFieldWithContextClassloaderLoosenType(
        readerFactory, "topicPartition") match {
      ...
      val customClusterName = ReflectionHelper.reflectFieldWithContextClassloader[
        java.util.Map[String, Object]](readerFactory, "kafkaParams") match {
        case Some(params) =>
